...
pub async fn chat(
    ...
    if chunks.is_empty() || best_rank < TUTOR_RANK_THRESHOLD {
        let answer = if profile.preferred_language.starts_with("ru") {
            "В базе знаний нет проверенной информации по этому вопросу. Попробуйте спросить про HTML, CSS, JavaScript, React, Git, TypeScript, UX-исследование, эвристики Нильсена, WCAG или Figma."
                .to_string()
        } else {
            "I don't have verified information on that in the knowledge base. Try asking about HTML, CSS, JavaScript, React, Git, TypeScript, UX research, Nielsen heuristics, WCAG, or Figma."
                .to_string()
        };
